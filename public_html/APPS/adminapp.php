<?php
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		if(!empty($toefl_score) && ($toefl_score > 120 || $toefl_score < 0)){
			$toefl_score = "";
			$toefl_scoreErr .= '<p style="color:red;">INVALID TOEFL exam subject </p>';
		}
			...
			*/
			...
			if(!(isset($_SESSION['aid']))){
				...
				if(empty($fname) || empty($lname) || empty($degree) || empty($email) || empty($admitSemes) || empty($admitYear) || empty($sid) || empty($dobirth) || empty($ssn) || empty($address) || empty($interest) || empty($experience) || !empty($fnameErr) || !empty($lnameErr) || !empty($degreeErr) || !empty($admitSemesErr) || !empty($admitYearErr) || !empty($sidErr) || !empty($dobirthErr) || !empty($ssnErr) || !empty($addressErr) || !empty($gre_verbalErr) || !empty($gre_quantitativeErr) || !empty($gre_yearErr) || !empty($gre_adv_scoreErr) || !empty($gre_adv_subjectErr) || !empty($gre_adv_yearErr) || !empty($toefl_scoreErr) || !empty($toefl_yearErr) || !empty($interestErr) || !empty($experienceErr) || !empty($emailErr) ) {
		            ...
		            $sql = "UPDATE application SET applyfor='$degree', priorDegree1='$priorDegree1', pd1Place='$pd1Place', pd1GPA='$pd1GPA', priorDegree2='$priorDegree2', pd2Place='$pd2Place',pd2GPA='$pd2GPA',priorExp='$experience', interest='$interest', admitYear=$admitYear, admitSemes='$admitSemes', submit='1', dateSubmission='$date' WHERE aid='$aid'";
		            $result = mysqli_query($db, $sql);
		            if($result){
		            	//echo "Success <br> ";
		            } else {
	            ...
	            if(empty($error)){
					$_SESSION['submit'] = 1;
					$_SESSION['appchangemsg'] = $fname . " " . $lname . "'s application is successfully updated.";
		...
		if(isset($_POST['save'])){
			...
			if(!(isset($_SESSION['username']))){
				...
				if(!empty($fnameErr) || !empty($lnameErr) || !empty($degreeErr) || !empty($admitSemesErr) || !empty($admitYearErr) || !empty($sidErr) || !empty($dobirthErr) || !empty($ssnErr) || !empty($addressErr) || !empty($priorDegree1Err) || !empty($priorDegree2Err) || !empty($gre_verbalErr) || !empty($gre_quantitativeErr) || !empty($gre_yearErr) || !empty($gre_adv_scoreErr) || !empty($gre_adv_subjectErr) || !empty($gre_adv_yearErr) || !empty($toefl_scoreErr) || !empty($toefl_yearErr) || !empty($interestErr) || !empty($experienceErr) || !empty($emailErr)){
		            ...
		            $sql = "UPDATE application SET applyfor='$degree', priorDegree1='$priorDegree1',pd1Place='$pd1Place', pd1GPA='$pd1GPA', priorDegree2='$priorDegree2',pd2Place='$pd2Place', pd2GPA='$pd2GPA', priorExp='$experience', interest='$interest', admitYear=$admitYear, admitSemes='$admitSemes' WHERE aid='$aid'";
					$result = mysqli_query($db, $sql);
		            if($result){
		            	//echo "Success <br> ";
		            } else {
		  	  	...
		  	  	<div class = "container">
 					  ...
 					  <h2 class= "card-header">APPLICANT INFORMATION</h2>
						...
						<label for="degree">Prior Degrees: </label><br>
						    ...
						    <input type="checkbox" onclick="pdFunc('pd1holder',this.checked)" class="form-check-input" name="priorDegree1" value="B" <?php if($priorDegree1 == 'B'){ echo "checked"; }?> >
						    <label class="form-check-label" for="B"> Bachelor's Degree </label>
							<div id="pd1holder" <?php if($priorDegree1 != 'B'){echo "style=\"display:none;\"";} else {echo "style=\"display:inline;\"";}?> >
								<input name="pd1Place" id="pd1Place" placeholder="Please specify where did you complete your Bachelor's Degree." style="width:500px;" type="text" value = <?php if($pd1Place != ""){echo '"'.$pd1Place.'"';}?> >
						    	<input name="pd1GPA" id="pd1GPA" placeholder="GPA" style="width:50px;" type="number" min = "0.00" max = "4.00" step="0.01" value = <?php if($pd1GPA != NULL){echo '"'.$pd1GPA.'"';}?>>
							</div><br>
							<?php echo $priorDegree2Err ?>      
						    <input type="checkbox" onclick="pdFunc('pd2holder', this.checked)" class="form-check-input" name="priorDegree2" value="MS" <?php if($priorDegree2 == 'MS'){ echo "checked"; }?>>
							<label class="form-check-label" for="exampleCheck1"> Master's Degree </label>
							<div id="pd2holder" <?php if($priorDegree2 != 'MS'){echo "style=\"display:none;\"";} else {echo "style=\"display:inline;\"";}?> >
						    	<input name="pd2Place" id="pd2Place" placeholder="Please specify where did you complete your Master's Degree." style="width:500px;margin-left:15px;" type="text" value=<?php if($pd2Place != ""){echo '"'.$pd2Place.'"';}?> >
								<input name="pd2GPA" id="pd2GPA" placeholder="GPA" style="width:50px;" type="number" min = "0.00" max = "4.00" step="0.01" value = <?php if($pd2GPA != NULL){echo '"'.$pd2GPA.'"';}?> type="hidden">
							</div><br><br>						
						</div>
						...
						<div class="form-check">
							  ...
							  <input type="radio" class="form-check-input" name="degree" value="MS" <?php if(strcasecmp($degree, 'MS') == 0){ echo 'checked=checked'; }?>>
							  <label class="form-check-label" for="ms" >MS</label><br>

							  <input type="radio" class="form-check-input" name="degree" value="PhD" <?php if(strcmp($degree, 'PhD') == 0){ echo 'checked=checked'; }?>>
						...
						<div class = "form-row">
							...
							<div class ="col">
							  ...
							  <select class="form-control" id="admitSemes" name="admitSemes">
								  ...
								  <option value="Fall" <?php if($admitSemes == 'Fall'){ echo 'selected="selected"'; }?> > Fall </option>
								  <option value="Spring" <?php if($admitSemes == 'Spring'){ echo 'selected="selected"'; }?> > Spring </option>
							  </select><br>
							</div>
							...
							<div class ="col">
							  ...
							  <select class="form-control" id="admitYear" name="admitYear">
								  ...
								  <option value=<?php echo '"' . $currentYear . '"'; ?> <?php if($admitYear == $currentYear){ echo 'selected="selected"'; }?> > <?php echo $currentYear; ?> </option>
								  <option value=<?php echo '"' . ($currentYear+1) . '"';  ?> <?php if($admitYear == ($currentYear+1)){ echo 'selected="selected"'; }?> > <?php echo $currentYear+1; ?> </option>
							  </select><br>
							</div>
